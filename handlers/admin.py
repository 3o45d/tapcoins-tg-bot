...
from filters.admin import AdminFilter, UrlFilter
...
async def start_handler(call: CallbackQuery, state: FSMContext) -> None:
    await state.clear()
...
async def change_image_handler(call: CallbackQuery, state: FSMContext) -> None:
...
async def confirm_image_handler(message: Message, state: FSMContext) -> None:
...
async def change_text_handler(call: CallbackQuery, state: FSMContext) -> None:
...
async def show_buttons_handler(call: CallbackQuery, state: FSMContext) -> None:
    await state.clear()
    await call.message.answer(
...
async def create_button_handler(call: CallbackQuery, state: FSMContext) -> None:
...
async def create_button_handler(message: Message, state: FSMContext) -> None:
...
@router.message(StateFilter(admin_states.AddButton.url), AdminFilter(), UrlFilter())
async def confirm_button_handler(message: Message, state: FSMContext) -> None:
...
async def on_button_handler(call: CallbackQuery, state: FSMContext) -> None:
...
@router.callback_query(
    StateFilter(admin_states.UpdateButton.on_item),
    admin_filters.ChangeTextButtonFilter(),
    AdminFilter()
)
async def update_button_text_handler(call: CallbackQuery, state: FSMContext) -> None:
...
@router.callback_query(
    StateFilter(admin_states.UpdateButton.on_item),
    admin_filters.ChangeLinkButtonFilter(),
    AdminFilter()
)
async def update_button_url_handler(call: CallbackQuery, state: FSMContext) -> None:
...
@router.callback_query(
    StateFilter(admin_states.UpdateButton.on_item),
    admin_filters.RemoveButtonFilter(),
    AdminFilter()
)
async def remove_button_handler(call: CallbackQuery) -> None:
    button_id = int(call.data.split("_")[-1])
...
async def confirm_button_text_handler(message: Message, state: FSMContext) -> None:
    current_data = await state.get_data()
...
@router.message(StateFilter(admin_states.UpdateButton.url), AdminFilter(), UrlFilter())
async def confirm_button_url_handler(message: Message, state: FSMContext) -> None:
    current_data = await state.get_data()
    button_id = current_data.get("button_id")
